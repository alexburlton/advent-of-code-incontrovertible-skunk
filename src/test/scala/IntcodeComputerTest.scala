...
class IntcodeComputerTest extends FlatSpec {
  ...
  "Position mode example from 5B" should "output 0 for input 0" in {
    val computer = new IntcodeComputer(List(3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9), 0)
    computer.process()
  ...
  "Position mode example from 5B" should "output 1 for non-zero input" in {
    val computer = new IntcodeComputer(List(3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9), 12)
    computer.process()
    assert(computer.outputs.toList == List(1))

    val computer2 = new IntcodeComputer(List(3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9), -5)
    computer2.process()
    assert(computer2.outputs.toList == List(1))
  }

  "Immediate mode example from 5B" should "output 0 for input 0" in {
    val computer = new IntcodeComputer(List(3,3,1105,-1,9,1101,0,0,12,4,12,99,1), 0)
    computer.process()
  ...
  "Immediate mode example from 5B" should "output 1 for non-zero input" in {
    val computer = new IntcodeComputer(List(3,3,1105,-1,9,1101,0,0,12,4,12,99,1), 12)
    computer.process()
    assert(computer.outputs.toList == List(1))

    val computer2 = new IntcodeComputer(List(3,3,1105,-1,9,1101,0,0,12,4,12,99,1), -5)
    computer2.process()
    assert(computer2.outputs.toList == List(1))
  }

  "Example 1 from 5B" should "output 1 for 8, 0 otherwise" in {
    val computer = new IntcodeComputer(List(3,9,8,9,10,9,4,9,99,-1,8), 8)
    computer.process()
    assert(computer.outputs.toList == List(1))

    val computer2 = new IntcodeComputer(List(3,9,8,9,10,9,4,9,99,-1,8), 7)
    computer2.process()
    assert(computer2.outputs.toList == List(0))
  }

  "Example 2 from 5B" should "output 1 for < 8, 0 otherwise" in {
    val computer = new IntcodeComputer(List(3,9,7,9,10,9,4,9,99,-1,8), 7)
    computer.process()
    assert(computer.outputs.toList == List(1))

    val computer2 = new IntcodeComputer(List(3,9,7,9,10,9,4,9,99,-1,8), 8)
    computer2.process()
    assert(computer2.outputs.toList == List(0))

    val computer3 = new IntcodeComputer(List(3,9,7,9,10,9,4,9,99,-1,8), 9)
    computer3.process()
    assert(computer3.outputs.toList == List(0))
  }

  "Example 3 from 5B" should "output 1 for 8, 0 otherwise" in {
    val computer = new IntcodeComputer(List(3,3,1108,-1,8,3,4,3,99), 8)
    computer.process()
    assert(computer.outputs.toList == List(1))

    val computer2 = new IntcodeComputer(List(3,3,1108,-1,8,3,4,3,99), 7)
    computer2.process()
    assert(computer2.outputs.toList == List(0))

    val computer3 = new IntcodeComputer(List(3,3,1108,-1,8,3,4,3,99), 9)
    computer3.process()
    assert(computer3.outputs.toList == List(0))
  }

  "Example 4 from 5B" should "output 1 for < 8, 0 otherwise" in {
    val computer = new IntcodeComputer(List(3,3,1107,-1,8,3,4,3,99), 7)
    computer.process()
    assert(computer.outputs.toList == List(1))

    val computer2 = new IntcodeComputer(List(3,3,1107,-1,8,3,4,3,99), 8)
    computer2.process()
    assert(computer2.outputs.toList == List(0))

    val computer3 = new IntcodeComputer(List(3,3,1107,-1,8,3,4,3,99), 9)
    computer3.process()
    assert(computer3.outputs.toList == List(0))
  }
  ...
  "Larger example from 5B" should "output 1001 if the input is greater than 8" in {
    val computer = new IntcodeComputer(List(3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,
      1106,0,36,98,0,0,1002,21,125,20,4,20,1105,1,46,104,
      999,1105,1,46,1101,1000,1,20,4,20,1105,1,46,98,99), 100)
